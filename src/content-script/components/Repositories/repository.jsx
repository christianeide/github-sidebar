...
export default function Repository (props) {
  const { data: { name, url, items, totalItems, owner }, type, numberOfItems, timeBeforeStale, sortBy, port } = props
  ...
  return (
    <li className='repository'>
      ...
      {items.length > 0 &&
        <ul>
          {items.map(item => {
            return <Item item={item} type={type} port={port} timeBeforeStale={timeBeforeStale} sortBy={sortBy} />
          })}
        </ul>
      }
    </li>
