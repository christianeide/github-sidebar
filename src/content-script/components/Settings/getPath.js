...
export function getCurrentPath (currentRepo, token) {
  ...
  if (GH_RESERVED_USER_NAMES.includes(owner) || GH_RESERVED_REPO_NAMES.includes(name)) {
    return
  }
...
export function canAddRepository (currentRepos) {
  ...
  return !currentRepos.find((currentRepo) => JSON.stringify(currentRepo) === JSON.stringify(validPage))
}
